> **【廃止 2026-09-07】** 本仕様 (v0.1 下書き) はオーナー判断で**破棄**された。後継は `SETUP_CONSOLE_SPEC.md`
> (セットアップコンソール)。■未決 1〜8 は議論されず、3本柱/ARMED/regime という枠組み自体を採用しない。
> `entry_state.json` の出力契約 (§2) はダッシュボードの Entry Engine パネルが引き続き読むため、
> スキーマの記録として本ファイルを残す。新規実装の根拠には使わないこと。
